...
export default function FollowersScreen({ route }) {
  ...
  const fetchFollowers = async (pageNum = 1) => {
    setLoading(true);
    try {
      const response = await api.getFollowers(userId, { page: pageNum, limit: 20 });
      setFollowers(response.followers || []);
      setPage(pageNum);
      setHasMore(response.followers && response.followers.length === 20);
    } catch (err) {
      ...
      {!loading && followers.length === 0 && (
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptySubText}>
            When users start following {user.username}, they'll appear here.
          </Text>
        </View>
      )}
